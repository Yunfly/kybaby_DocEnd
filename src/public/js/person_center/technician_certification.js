...
export default function (obj) {
  ...
  //证书
  $("#updateMyHeadFileElem").change(function (event) {
    if(disable) {
...
function gettittle(){
	$.ajax({
		...
		success:function(result){
			...
			for(var k=0;k<result.goodFieldList.length;k++){
				if(result.goodFieldList[k].doctorType=="技师") {
					$("#goodFieldList").append("<li><span class='grey' goodFieldId='" + result.goodFieldList[k].id + "'>" + result.goodFieldList[k].name + "</span></li>");
				}
			}
      ...
      $("#goodFieldList li .grey").click(function () {
        if(disable) {
          changesymptom($(this));
        }
      });
	...
	$.ajax({
		...
		success:function(result){
			...
			if(doctor.doctorImage == ''){
				$('#showDoctor_img').attr("src",urlWay.fileserver+"/kybabyBG/admin/images/userFaceIcon/lt_doctor.png");
			}else{
				$('#showDoctor_img').attr("src",urlWay.fileserver+doctor.doctorImage);
			}
