      ...
      $(".levelTwo input[type!='checkbox'],.levelTwo select,.levelTwo textarea").each(function(){
        ...
        if($(this).attr("placeholder")!=undefined){
          if($(this).hasClass("mustNumber")){
            ...
            else if($(this).attr("id")=='motherName'){
              if(val.length>4){
                alert($(this).attr("placeholder")+'这一栏填写字符不超过4个');
                $(this).removeAttr('readonly').removeAttr("disabled");
                num++;
              }
            }
            else if($(this).attr("id")=='heightOfMother'){
              if(val.length>6){
                alert($(this).attr("placeholder")+'这一栏填写字符不超过6个');
                $(this).removeAttr('readonly').removeAttr("disabled");
                num++;
              }
            }
...
//		}
    ...
    getOrderInfo:function(){//获取订单信息
      ...
      $.ajax({
        ...
        success:function(result){
          ...
          $("#product_info img").attr("src",urlWay.fileserver+result.product.smallPicture);
          ...
          if(result.orderNodeTrack == null){
            ...
            _this.step = result.orderNodeTrack.flowNodeId;
          }
          _this.getFlowNode();
          _this.eventOnLoad();
    ...
    updateHealthRecord:function(){//保存健康档案
      var _this=this;
      var recordHealth=_this.recordHealthData();
        ...
        success:function(result){
          ...
          $.each(result.flowNodeFoList,function(key,val){//先遍历flowNodeFoList
            ...
            $.each(val.itemAndResultList,function(key2,val2){//再遍历每一页具体的表单
              ...
              if(val2.resultType == "输入"){
              ...
              }else if(val2.resultType == "多选"){//new by yjl
                var box = val2.options.split(",");//拆分多选选项
                var box_html='';
                var value=val2.resultValue;
                ...
                for(var m= 0;m<len;m++){
                  var more_check='';
                  if($.inArray(box[m],value)>-1){//是否有选中值
                    more_check='more_check';
                  }
                  box_html+='<p class="'+more_check+'">'+box[m]+'</p>';
                }
                $(".item-"+(key+1)).find("ul").append(
                  "<li style='height:"+(len*40)+"px' class='more_select'" +
                  " data-itemId='"+val2.itemId+"' "+
                  " data-itemResultName='"+val2.itemResultName+"' "+
                  " data-resultUnit='"+val2.resultUnit+"' "+
                  " data-resultValueId='"+val2.resultValueId+"' "+
                ...
                $('.more_select p').unbind().click(function () {
                  $(this).toggleClass('more_check');
                })
          ...
          if(_this.step == "first"){
            ...
            _this.initialize((Number(node)+1));
          }
          if(islastmark){
            $.ajax({
              url:urlWay.orderHost+'orderManager.action?whoLoginFlag=doctor',
              ...
              success:function(result){
                ...
                $(".resultRecordingStep:last").find("ul").append(
                  ...
                  "<input class='healthResultSelectAll' id='selectall' data-type='selectall' type='checkbox'>"+
                  "<label for='selectall'>"+
                  "<h6>全选</h6>"+
                  "<p></p>"+
                  "</label>"+
                ...
                $('.healthResultSelectAll').unbind().change(function (e) {
                  _this.selectAll(this,e);
                });
              }
            });
          }
    ...
    updateResultData:function(){//更新或插入结果数据
      var Data = {};
      var jsonArr = [];
      $(".resultRecordingStep:visible input,.resultRecordingStep:visible select,.more_select").each(function(){
        var jsonData = {};
        ...
        if($(this).hasClass("more_select")){
          var options=$(this).find('.more_check');
          var values='';
          for(var i= 0,len=options.length;i<len;i++){
            values+=','+$(options).eq(i).text();
          }
          if(values!=''){
            values=values.substring(1);
          }
          jsonData.resultValue = values;
        }else{
    ...
    updateHealthDirect:function(){//更新健康指导
      ...
      Data["orderInfo.id"] = _this.searchId;
