<template>
  <div>
    <div id="header" class="doctor_header">
      ...
      <div class="header_center font18">计免记录</div>
        ...
        <div class="list_model_group">
          <div class="list_model_cont_box">
            <div class="list_model_item" data-type="off">
              <ul>
                <li class="list_model_cont font10 paragraph" v-for="item in user_count_list">
                  <p class="gray_line"></p>
                  <div class="list_model flex_cont">
                    <p class="flex_item">{{item.organInoculationOpenResources.openDate}}</p>
                    <p class="user_ml10">{{getMonth(item.userInfo.birthday, item.organInoculationOpenResources.openDate)}}个月</p>
                  </div>
                </li>
              </ul>
            </div>
...
<script>
  ...
  export default {
    ...
    created : function () {
      ...
      this.getUserCountList(this.user_id);
    },
    mounted : function () {

    },
    methods : {
      getUserCountList : function (id) {
        var that = this;
        this.$ajax.post(urlWay.fdmanage + 'fdUserManage.action',"action=getUserInoculationAppointmentInfoList&userInfo.id="+id)
          .then(function(result){
            if(result.data.mes == "成功") {
              that.user_count_list = result.data.userInoculationAppointmentInfoList;
      ...
      getMonth : function (oldMonth, newMonth) {
        var old_month = oldMonth.split("-");
        var new_month = newMonth.split("-");
        var months = null;

        months = (new_month[0] - old_month[0]) * 12 + (new_month[1] - old_month[1]);
