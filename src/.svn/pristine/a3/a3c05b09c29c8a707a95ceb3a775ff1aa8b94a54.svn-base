  ...
  <div>
    <div class="header_box gradient_bg">
      ...
      <div class="content_header flex_cont content_center">
        <div class="content_logo font10" v-if="online" v-on:click="receiveOrder(true)" >当前在线接单</div>
    ...
    <div id="container" class="container_box">
      <section id="consult">
        <ul v-for="(item,index) in consult_sign" :attr="sign_type[index].userType"  v-show="showAll || (sign_type[index].userType==statusNOW)||(sign_type[index].userType==statusElse)" >
          <router-link :to="{path:'/consulting/consulting_detail',query: {id:sign_type[index].logId,packageId:sign_type[index].packageId,isTag:sign_type[index].ifShow,userId:sign_type[index].userId,userType:sign_type[index].userType}}" >
            <li class="flex_cont flex_simple">
              ...
              <div class="flex_item">
                <div class="flex_cont">
                  <p class="consult_name flex_item">{{item.name}}   {{sign_type[index].userType}}</p>
                ...
                <div class="flex_cont">
                  ...
                  <p class="consult_state font10 color_txt" v-if="sign_type[index].ifShow">待添加病情</p>
                </div>
              </div>
            </li>
          </router-link>
  ...
  export default{
    ...
    methods: {
      receiveOrder:function(onOrder){
          ...
          param.append('action','saveOrUpdateConsultDoctorInfo');
          param.append('consultDoctorInfo.id',this.consultDoctorId);
          param.append('consultDoctorInfo.isOnline',this.online);
          const config = { headers: { 'Content-Type': 'multipart/form-data' } };
          this.$ajax.post(host+'consultManage.action',param,config)
            .then(function(res){
            ...
            if(result.data.mes=="成功") {
              ...
              for(var i = 0; i < result.data.someCousultStrList.length; i++) {
                ...
                type.push({
                  ...
                  "ifAdd" : result.data.someCousultStrList[i].split("::")[6],
                  //以要显示为判断依据   "ifShow" : result.data.someCousultStrList[i].split("::")[7] =='要显示'?true:false,
