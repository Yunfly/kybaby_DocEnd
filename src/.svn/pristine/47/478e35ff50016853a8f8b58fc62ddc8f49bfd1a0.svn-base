<template>
  <div  class="bgfff">
    <div id="header" class="doctor_header">
      ...
      <div class="header_center font18">门诊订单</div>
      <div class="header_right">
        <div class="more_btn"></div>
      </div>
      <div class="cf"></div>
      <div class="order_btn">
        <div class="active"><router-link to="/outpatient_order">儿科订单</router-link></div>

        <div><router-link to="/his_order">儿保订单 </router-link></div>
    ...
    <div id="container_full" class="bgfff">
      ...
      <section>
        <ul id="all_list" class="list_box">
          <li v-for="item in orders"   v-show="showAll || (item.orderStatus==statusNOW)||item.orderStatus==statusElse">
            <div class="flex_cont">
              <div class="head_img flex_item">
                <img  v-bind:src=imgUrl+item.userInfo.userImage  v-if="item.userInfo.userImage!=undefined"   />
                <p class="font10">{{item.userInfo.babyName}}</p>
              </div>
              <div class="flex_item flex_simple">
                <p class="li_date font10 ">门诊日期 : <span>{{item.orderTime.substring(0,10)}}</span></p>
                <p class="li_time font10">门诊时间：{{item.appointmentBeganTime}}-{{item.appointmentEndTime}}</p>
                <p class="li_address font10 ">{{item.clinicAddress}}</p>
              </div>
              <div class="flex_item flex_simple">
...
<script>
 ...
 export default{
   ...
   mounted:function(){
     var that = this;
     this.$ajax.post(urlWay.clinicHost+'doctorClinicOrder.action','action=getClinicOrderList')
       .then(function(result){
         that.orders = result.data.orderInfoClinicList;
