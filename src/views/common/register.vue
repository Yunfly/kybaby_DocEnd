    ...
    methods:{
          ...
          $.ajax({
            ...
            success : function(result) {
              if(result.mes=="已注册"){
                ...
                that.$router.push('/service/roleselect');
            }
            }
          });
        }
      ...
      numReduce:function (){
        ...
        (function num(){
            ...
            if(ttt <= 0){
              ttt = 60;
              $('#get_reg_code').attr('onclick','httpPost()').text('获取验证码').css({'backgroundColor':'#50c1e9','pointer-events':'auto'});
            }else{
              $('#get_reg_code').removeAttr('onclick').css({'backgroundColor':'#dadada','pointer-events':'none'})
