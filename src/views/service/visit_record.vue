    ...
    <div id="header" class="doctor_header">
      ...
      <div>
        <nav class="flex_cont font12">
          ...
          <div class="tag_menu flex_item" v-on:click="change('未完成',1)">未完成</div>
          <div class="tag_menu flex_item" v-on:click="change('已完成',2)">已完成</div>
        </nav>
    ...
    <div id="container_full" class="bgfff">
      <section id="visit_record">
        <div v-show="orderType=='儿科'">
          <ul class="all_list" v-show="liState=='全部'" v-bind:class="menu_slide">
            <li v-for="item in orders">
              ...
              date:item.clinicDate,time:item.clinicBeganTime+'-'+item.clinicEndTime,type:0}}">
                <div class="flex_cont">
          ...
          <ul class="unfinished_list" v-show="liState=='未完成'" v-bind:class="menu_slide">
            <li v-for="item in orders" v-if="item.orgClinicStatus=='上班中'||item.orgClinicStatus=='未开始'
          ||item.orgClinicStatus=='请签到'||item.orgClinicStatus=='未签到'||item.orgClinicStatus=='未签退'">
              <router-link v-bind:to="{path:'/service/visit_details',query:{id:item.doctorMorePracticeOrgInfo.id,
              doctor_id:item.doctorInfo.id,data_id:item.id,status:item.orgClinicStatus,name:item.clinicOrg,
              date:item.clinicDate,time:item.clinicBeganTime+'-'+item.clinicEndTime,type:0}}">
                <div class="flex_cont">
                ...
                <div class="record_type in_work" v-if="item.orgClinicStatus=='上班中'"></div>
                <div class="record_type not_at" v-else-if="item.orgClinicStatus=='未开始'"></div>
                <div class="record_type please_sign" v-else-if="item.orgClinicStatus=='请签到'"></div>
                <div class="record_type not_sign" v-else-if="item.orgClinicStatus=='未签到'"></div>
                <div class="record_type not_sign_back" v-else-if="item.orgClinicStatus=='未签退'"></div>
          ...
          <ul class="complete_list" v-show="liState=='已完成'" v-bind:class="menu_slide">
            <li v-for="item in orders" v-if="item.orgClinicStatus=='已完成'">
              <router-link v-bind:to="{path:'/service/visit_details',query:{id:item.doctorMorePracticeOrgInfo.id,
              doctor_id:item.doctorInfo.id,data_id:item.id,status:item.orgClinicStatus,name:item.clinicOrg,
              date:item.clinicDate,time:item.clinicBeganTime+'-'+item.clinicEndTime,type:0}}">
                <div class="flex_cont">
        ...
        <div v-show="orderType=='儿保'" id="child">
          <ul class="all_list" v-show="liState=='全部'" v-bind:class="menu_slide">
            <li v-for="vm in childOrders">
              <router-link v-bind:to="{path:'/service/visit_details',query:{id:vm.organChildcareOpenResources.id,
              doctor_id:vm.doctorInfo.id,data_id:vm.id,status:vm.workStatus,
              name:vm.organChildcareOpenResources.hospitalBasicInfo.hospitalLname,
              date:vm.organChildcareOpenResources.openDate,time:vm.organChildcareOpenResources.openStartTime+'-'+vm.organChildcareOpenResources.openEndTime,type:1}}">
                <div class="flex_cont">
                ...
                <div class="record_type completed" v-if="vm.workStatus=='已完成'"></div>
                <div class="record_type in_work" v-else-if="vm.workStatus=='上班中'"></div>
                <div class="record_type not_at" v-else-if="vm.workStatus=='未开始'"></div>
                <div class="record_type not_sign" v-else-if="vm.workStatus=='未签到'"></div>
                <div class="record_type not_sign_back" v-else-if="vm.workStatus=='未签退'"></div>
                <div class="record_type please_sign" v-else-if="vm.workStatus=='请签到'"></div>
              </router-link>
            </li>
          </ul>
          <ul class="unfinished_list" v-show="liState=='未完成'" v-bind:class="menu_slide">
          ...
          ||vm.workStatus=='未签到'||vm.workStatus=='请签到'||vm.workStatus=='未签退'">
              ...
              date:vm.organChildcareOpenResources.openDate,time:vm.organChildcareOpenResources.openStartTime+'-'+vm.organChildcareOpenResources.openEndTime,type:1}}">
                ...
                <div class="record_type in_work" v-if="vm.workStatus=='上班中'"></div>
                <div class="record_type not_at" v-else-if="vm.workStatus=='未开始'"></div>
                <div class="record_type please_sign" v-else-if="vm.workStatus=='请签到'"></div>
                <div class="record_type not_sign" v-else-if="vm.workStatus=='未签到'"></div>
                <div class="record_type not_sign_back" v-else-if="vm.workStatus=='未签退'"></div>
...
//        if(index != this.idx){
//          $(".tab_logo_box").css('marginLeft', ($(this).index())*33.3+"%").animateCss(($(this).index() < this.idx ? "bounceInRight" : "bounceInLeft"));
//          $("section").children("ul").eq($(this).index()).siblings().hide();
//          $("section").children("ul").eq($(this).index()).show().animateCss(($(this).index() < this.idx ? "bounceInRight" : "bounceInLeft"));
//          this.idx = index;
//        }
        this.idx = index;
        if(state=="未完成"){
